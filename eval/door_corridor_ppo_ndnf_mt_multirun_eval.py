...
from eval.common import FailureCode
...
def post_training(model_dir: Path, model: DCPPONDNFMutexTanhAgent):
    ...
    if is_truncated(og_ndnf_mt_logs):
        ...
        return FailureCode.FAIL_AT_EVAL_NDNF_MT_TRUNCATED
    if not is_truncated(og_ndnf_mt_logs) and is_truncated(og_ndnf_mt_dis_logs):
        ...
        return FailureCode.FAIL_AT_EVAL_NDNF_MT_DIS_TRUNCATED
    if og_ndnf_mt_dis_logs["missing_actions"]:
        log.info("NDNF MT dis has missing actions!")
        return FailureCode.FAIL_AT_EVAL_NDNF_MT_DIS_MISS_ACTION
    ...
    while not terminated and not truncated:
        ...
        if len(all_answer_sets) != 1:
            ...
            return FailureCode.FAIL_AT_RULE_EVAL_NOT_ONE_ANSWER_SET

        if all_answer_sets[0] == "":
            log.info(f"No output action!")
            return FailureCode.FAIL_AT_RULE_EVAL_MISSING_ACTION
        ...
        if len(output_classes_set) != 1:
            ...
            return FailureCode.FAIL_AT_RULE_EVAL_MORE_THAN_ONE_ACTION
    ...
    if truncated:
        ...
        return FailureCode.FAIL_AT_RULE_EVAL_TRUNCATED
...
def post_train_eval(eval_cfg: DictConfig):
    ...
    ret_dict: dict[int, list[int]] = dict(
        [(c.value, []) for c in FailureCode] + [(0, [])]
    )
    ...
    for k, v in ret_dict.items():
        if k == 0:
            ...
            log.info(f"{FailureCode(k).name}: {sorted(v)}")
