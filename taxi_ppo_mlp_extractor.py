...
from common import synthesize, init_params
...
class TaxiEnvPPOBaseAgent(nn.Module):
    ...
    def _create_default_actor(self) -> nn.Module:
        return nn.Sequential(
            nn.Linear(64, self.actor_latent_size),
            nn.Tanh(),
...
class TaxiEnvPPONDNFAgent(TaxiEnvPPONDNFBasedAgent):
    ...
    def _create_default_actor(self) -> NeuralDNF:
        return NeuralDNF(64, self.actor_latent_size, self.action_size, 1.0)
...
class TaxiEnvPPONDNFEOAgent(TaxiEnvPPOBaseAgent):
    ...
    def _create_default_actor(self) -> NeuralDNFEO:
        return NeuralDNFEO(64, self.actor_latent_size, self.action_size, 1.0)
...
def construct_model(
    ...
    if not use_ndnf:
        return TaxiEnvPPOMLPAgent(
            use_decode_obs=use_decode_obs, actor_latent_size=actor_latent_size
        )
